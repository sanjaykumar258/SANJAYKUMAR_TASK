...
├── dataset_evaluation.py   # Pipeline evaluations on standard datasets
└── README.md
```
...
## 📈 4. Dataset Evaluation — `dataset_evaluation.py`

Evaluates the generation and sentiment pipelines against standard HuggingFace datasets (`imdb` for sentiment and `ag_news` for text generation). 
...
# Specify task
python dataset_evaluation.py --task sentiment
python dataset_evaluation.py --task generation

# Specify number of samples
python dataset_evaluation.py --samples 5
```
...
| Argument    | Type  | Default  | Description                                        |
|-------------|-------|----------|----------------------------------------------------|
| `--task`    | str   | `"both"` | Task to run (`"sentiment"`, `"generation"`, `"both"`) |
| `--samples` | int   | `3`      | Number of dataset samples to evaluate              |
